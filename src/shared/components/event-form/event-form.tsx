...
function useHandleSubmit(event?: EventType) {
    ...
    const createEvent = async (data: any) => {
        await axios.post(EVENTS_API, data);
    };

    const updateEvent = async (payload: any) => {
        const { body, params: { id } } = payload
...
export default function EventForm(props: props) {
    ...
    const { fields, append } = useFieldArray({ name: "coordinators", control: form.control, })

    const facultyFields = useMemo(() => fields.filter((field) => field.type === 'FACULTY'), [fields]);
    const studentFields = useMemo(() => fields.filter((field) => field.type === 'STUDENT'), [fields]);
    const [images, setImages] = useState<{ url: string, name: string }[]>([]);

    useEffect(() => { if (isSuccess) router.push("/") }, [isSuccess]);

    useEffect(() => { if (event) setImages(event.images) }, [event]);

    return (
        <div className="space-y-6">

            <div>
                <h3 className="text-lg font-medium">{event ? 'UPDATE EVENT' : 'CREATE EVENT'}</h3>
                ...
                <form onSubmit={form.handleSubmit((data) => {
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input placeholder="Enter name" name={field.name} defaultValue={field.value} onChange={field.onChange} onBlur={field.onBlur} />
                                </FormControl>
                                <FormDescription>
                                ...
                                <Select onValueChange={field.onChange} defaultValue={field.value}>
                                    ...
                                    <SelectContent>
                                        <SelectGroup>
                                            ...
                                            <SelectItem value="MARKETINGCLUB">MARKETING CLUB</SelectItem>
                                        </SelectGroup>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-between items-center">
                            <span className="text-sm font-medium leading-none">FACULTY COORDINATORS</span>
                            <Button
                                type="button"
                                variant="default"
                                size="default"
                                onClick={() => append({ name: "", contNo: "", email: "", type: "FACULTY", department: "IT" })}
                            >
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {facultyFields.length ? facultyFields.map((field, index) => (
                                    <TableRow key={index}>
                                        <TableCell>
                                            <FormField
                                                //@ts-ignore
                                                name={`coordinators[${fields.findIndex((e) => e.id === field.id)}].name`}
                                                control={form.control}
                                                defaultValue=""
                                                render={({ field }) => (
                                                    <FormItem>
                                        ...
                                        <TableCell>
                                            <FormField
                                                //@ts-ignore
                                                name={`coordinators[${fields.findIndex((e) => e.id === field.id)}].email`}
                                                control={form.control}
                                                defaultValue=""
                                                render={({ field }) => (
                                                    <FormItem>
                                        ...
                                        <TableCell>
                                            <FormField
                                                //@ts-ignore
                                                name={`coordinators[${fields.findIndex((e) => e.id === field.id)}].contNo`}
                                                control={form.control}
                                                defaultValue=""
                                                render={({ field }) => (
                                                    <FormItem>
                                        ...
                                        <TableCell>
                                            <FormField
                                                //@ts-ignore
                                                name={`coordinators[${fields.findIndex((e) => e.id === field.id)}].department`}
                                                control={form.control}
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormControl>
                                                            ...
                                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                                ...
                                                                <SelectContent>
                                                                    ...
                                                                    <SelectItem value="COMMERCE">Commerce Department</SelectItem>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-between items-center">
                            <span className="text-sm font-medium leading-none">STUDENT COORDINATORS</span>
                            <Button
                                type="button"
                                variant="default"
                                size="default"
                                onClick={() => append({ name: "", email: "", contNo: "", type: "STUDENT", course: "BCA", semister: "1", })}
                            >
                                <AiOutlinePlusCircle className="mr-2 h-4 w-4" />
                                Add New
                            </Button>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {studentFields.length ? studentFields.map((field, index) => (
                                    <TableRow key={index}>
                                        <TableCell>
                                            <FormField
                                                //@ts-ignore
                                        ...
                                        <TableCell>
                                            <FormField
                                                //@ts-ignore
                                                name={`coordinators[${fields.findIndex((e) => e.id === field.id)}].course`}
                                                control={form.control}
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormControl>
                                                            ...
                                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                                ...
                                                                <SelectContent>
                                                                    ...
                                                                    <SelectItem value="BBA LLB">BBA LLB</SelectItem>
                                                                    <SelectItem value="B.COM LLB">B.COM LLB</SelectItem>
                                                                    <SelectItem value="B.A.LLB">B.A.LLB</SelectItem>
                                        ...
                                        <TableCell>
                                            <FormField
                                                //@ts-ignore
                                                name={`coordinators[${fields.findIndex((e) => e.id === field.id)}].semister`}
                                                control={form.control}
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormControl>
                                                            ...
                                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                                ...
                                                                <SelectContent>
                                                                    <SelectItem value="1">I<sup>st</sup></SelectItem>
                                                                    <SelectItem value="2">II<sup>nd</sup></SelectItem>
                                                                    <SelectItem value="3">III<sup>rd</sup></SelectItem>
                                                                    <SelectItem value="4">IV<sup>th</sup></SelectItem>
                                                                    <SelectItem value="5">V<sup>th</sup></SelectItem>
                                                                    <SelectItem value="6">VI<sup>th</sup></SelectItem>
                                                                    <SelectItem value="7">VII<sup>th</sup></SelectItem>
                                                                    <SelectItem value="8">VIII<sup>th</sup></SelectItem>
                                                                    <SelectItem value="9">IX<sup>th</sup></SelectItem>
                                                                    <SelectItem value="10">X<sup>th</sup></SelectItem>
                                                                </SelectContent>
                                                            </Select>
                                                        </FormControl>
                            ...
                            <FormItem className="space-y-4">
                                ...
                                <FormControl>
                                    <Upload
                                        ...
                                        multiple={false}
                                        accept=".docx"
                                        title={event ? 'Click here to change event report' : 'Click here to upload event report'}
                                        icon={<HiOutlineDocumentReport className="text-6xl mr-2" strokeWidth="1" />}
                                        onChange={(e) => {
                                            convertFilesToBase64Array(e.target.files, (base64Array: any) => {
                                                field.onChange(base64Array[0]); // Array of base64 encoded files
                                            })
                    ...
                    <div className="flex justify-end">
                        <Button type="submit" disabled={isLoading} isLoading={isLoading}>{event ? 'Update Event' : 'Create Event'}</Button>
                    </div>
                </form>
            </Form>
        </div>
