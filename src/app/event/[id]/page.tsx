...
export default function EventPage({ params, searchParams: { id }, }: { params: { id: string }, searchParams: { [key: string]: string } }) {

    const { data: event, isLoading, isError, error } = useQuery(['event', id], () => fetchEvent(id));

    const studentCoordinator = useMemo(() => {
        return (
            event?.coordinators.filter((coordinator: event_coordinator) => coordinator.type === 'STUDENT')
        )
    }, [event]);

    const facultyCoordinator = useMemo(() => {
        return (
            event?.coordinators.filter((coordinator: event_coordinator) => coordinator.type === 'FACULTY')
        )
    }, [event])
