...
async function getImagesZip(id: string) {
    try {
        const response: AxiosResponse<APIResponse<{ url: string, zipName: string }>> = await axios.get(IMAGES_ZIP_API.replace(':id', id))
...
const ImageDownload = ({ params: { id } }: { params: { id: string } }) => {
    ...
    useEffect(() => { if (data) window.open(data.url); window.close() }, [data])

    useEffect(() => { if (isError) throw new Error() }, [isError])


    return (
        <div className='absolute top-0 left-0 bg-white h-screen w-screen flex justify-center items-center'>
