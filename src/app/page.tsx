...
import { AppDispatch, store } from '@/shared/rdx/store'
import { addBreadCrumbItem } from '@/shared/components/ui/breadcrumb/breadcrumb.slice'

const getEvents = async (eventsType: string | undefined = undefined) => {

  try {
    const endPoint = eventsType === "all" ? EVENTS_API : RECENT_EVENTS_API
    const response: AxiosResponse<APIResponse<event[]>> = await axios.get(endPoint);
